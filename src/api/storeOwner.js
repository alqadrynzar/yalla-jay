        ...
        try {
            const query = `
                SELECT DISTINCT
                    ...
                    sr.support_phone_number
                FROM
                    stores s
                JOIN
                    store_service_regions ssr ON s.id = ssr.store_id
                JOIN
                    service_regions sr ON ssr.region_id = sr.id
                WHERE
                    s.owner_id = $1 AND sr.support_phone_number IS NOT NULL AND sr.support_phone_number <> '';
            `;
            const result = await client.query(query, [ownerId]);
            res.status(200).json({
                message: 'تم استرجاع قائمة جهات الاتصال بنجاح.',
                contacts: result.rows
            });
        } catch (err) {
            console.error("Error fetching owner's support contacts:", err);
            res.status(500).json({ message: 'حدث خطأ في الخادم أثناء جلب جهات الاتصال.' });
        } finally {
            client.release();
        }
    }
